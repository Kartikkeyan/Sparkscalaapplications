...
object twitterstream {
  def main(args: Array[String]) {
    //val spark = SparkSession.builder.master("local[*]").appName("twitterstream").config("spark.sql.warehouse.dir", "/home/hadoop/work/warehouse").enableHiveSupport().getOrCreate()
    val spark = SparkSession.builder.master("local[2]").appName("twitterstream").getOrCreate()
    ...
    val Accesstokensecret = "lvx5djMdO1XLQlNvOBISPlJYqsoUCx2y7UdcpeznnN89M" //Access token
